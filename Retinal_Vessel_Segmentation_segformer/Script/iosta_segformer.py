...
IMG_DIR   = Path(r"C:\Users\SC\Documents\Data and Code\Iosta datasets\Train\Images")
MASK_DIR  = Path(r"C:\Users\SC\Documents\Data and Code\Iosta datasets\Train\Labels")  # or ...\labels-vk
OUT_ROOT  = Path(r"C:\Users\SC\Documents\Data and Code\Iosta datasets\outputs_segformer_cv")
# ------------------------------------------
...
def pil_read_mask(path: Path) -> np.ndarray:
    """Return binary mask (0/1) from IOSTAR labels (white vessels on black)."""
    with Image.open(path) as im:
        im.load()
        if im.mode != "L":
            im = im.convert("L")
        arr = np.array(im)
        return (arr > 127).astype(np.uint8)  # threshold to {0,1}

def pair_iostar(img_dir: Path, mask_dir: Path):
    """
    IOSTAR structure (per your screenshots):
      .../Train/Images/im0001.jpg  ↔  .../Train/Labels/im0001.png
    Pair case-insensitively by stem; extensions may differ.
    """
    ...
    if miss:
        print(f"[WARN] {miss} image(s) in {img_dir} had no matching mask in {mask_dir} and were skipped.")
    return pairs_img, pairs_mask

# --- use it ---
# Set these to your IOSTAR folders, e.g.:
# IMG_DIR = r"C:\Users\SC\Documents\Data and Code\iosta datasets\Train\Images"
# MASK_DIR = r"C:\Users\SC\Documents\Data and Code\iosta datasets\Train\Labels"

all_imgs, all_masks = pair_iostar(IMG_DIR, MASK_DIR)
print(f"Total paired images: {len(all_imgs)}")
assert len(all_imgs) > 0, "No image/mask pairs found. Check IMG_DIR/MASK_DIR and stems (e.g., im0001)."
...
for fold_dir in sorted(OUT_ROOT.glob("fold_*")):
    ...
    # (Optional) small warm-up for stable GPU timing
    if DEVICE.type == "cuda":
        with torch.no_grad():
    ...
    # Per-fold CSV (per-image dice + time)
    ...
    # Per-fold summary (mean dice across images with GT, mean latency)
    mean_dice = df_fold["dice"].dropna().mean()
...
if all_rows:
    pd.DataFrame(all_rows).to_csv(OUT_ROOT / "test_metrics_all_folds.csv", index=False)
    print("Saved:", OUT_ROOT / "test_metrics_all_folds.csv")

if all_summaries:
    pd.DataFrame(all_summaries).to_csv(OUT_ROOT / "test_summary_all_folds.csv", index=False)
