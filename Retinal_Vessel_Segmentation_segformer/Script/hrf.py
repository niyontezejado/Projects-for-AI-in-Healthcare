...
IMG_DIR   = Path(r"C:\Users\SC\Documents\Data and Code\Hrf dataset\images")
MASK_DIR  = Path(r"C:\Users\SC\Documents\Data and Code\Hrf dataset\manual1")  # or ...\labels-vk
OUT_ROOT  = Path(r"C:\Users\SC\Documents\Data and Code\Hrf dataset\outputs_segformer_cv")
# ------------------------------------------
...
def pil_read_mask(path: Path) -> np.ndarray:
    """Return binary mask (0/1) from HRF labels."""
    with Image.open(path) as im:
        im.load()
        if im.mode != "L":
            im = im.convert("L")
        arr = np.array(im)
        # HRF masks are white vessels on black background; threshold at 128
        return (arr > 127).astype(np.uint8)

def pair_hrf(img_dir: Path, mask_dir: Path):
    """
    HRF files are like: 01_dr, 01_g, 01_h ...
    Masks use the SAME stem as images, but extensions may differ.
    This pairs by stem (case-insensitive) and accepts any common image extension.
    """
    # collect image paths
    imgs = sorted([p for p in img_dir.iterdir() if is_img(p)], key=lambda p: p.stem.lower())

    # index masks by stem (case-insensitive)
    mask_candidates = [p for p in mask_dir.iterdir() if is_img(p)]
    mask_by_stem = {p.stem.lower(): p for p in mask_candidates}

    pairs_img, pairs_mask = [], []
    miss = 0
    for ip in imgs:
        stem = ip.stem.lower()
        m = mask_by_stem.get(stem, None)
        if m is None:
            miss += 1
        else:
            pairs_img.append(ip)
...
# --- Use it ---
IMG_DIR = Path(IMG_DIR)   # if your variables are strings, wrap with Path
MASK_DIR = Path(MASK_DIR)

all_imgs, all_masks = pair_hrf(IMG_DIR, MASK_DIR)
print(f"Total paired images: {len(all_imgs)}")
assert len(all_imgs) > 0, "No image/mask pairs found. Check IMG_DIR / MASK_DIR and stems."
...
for fold_dir in sorted(OUT_ROOT.glob("fold_*")):
    ...
    with torch.no_grad():
        for xb, yb, ipath, gpath in dl_test:
            ...
            if DEVICE.type == "cuda":
                torch.cuda.synchronize()
            elapsed_ms = (time.perf_counter() - start_time) * 1000.0  # inference time in ms
    ...
    mean_time = df_fold["inference_time_ms"].mean()
    summary_row = {
        "fold": fold_dir.name,
        "mean_dice": mean_dice,
    ...
    pd.DataFrame([summary_row]).to_csv(fold_dir / "test_summary.csv", index=False)
