...
IMG_DIR   = Path(r"C:\Users\SC\Documents\Data and Code\store data\images")
MASK_DIR  = Path(r"C:\Users\SC\Documents\Data and Code\store data\Labels-ah")  # or ...\labels-vk
OUT_ROOT  = Path(r"C:\Users\SC\Documents\Data and Code\store data\store_segformer")
# ------------------------------------------
...
def pil_read_mask(path: Path) -> np.ndarray:
    """Return binary mask (0/1) from STARE labels."""
    with Image.open(path) as im:
        im.load()
        arr = np.array(im)
        if arr.ndim == 3:
...
def pair_stare(img_dir: Path, mask_dir: Path):
    """
    STARE images 'imXXXX.ppm'
    Labels often 'imXXXX.ah.ppm' or 'imXXXX.vk.ppm' (we pass mask_dir to pick which set).
    """
    imgs = sorted([p for p in img_dir.iterdir() if is_img(p) and p.stem.startswith("im")])
    pairs_img, pairs_mask = [], []
    miss = 0
    for ip in imgs:
        base = ip.stem  # e.g., im0001
        # Guess possible label names inside mask_dir
        candidates = [
            mask_dir / f"{base}.ah{ip.suffix}",
            mask_dir / f"{base}.vk{ip.suffix}",
            mask_dir / f"{base}.ah.ppm",
            mask_dir / f"{base}.vk.ppm",
            mask_dir / f"{base}{ip.suffix}",  # sometimes same name
        ]
        gp = None
        for c in candidates:
            if c.exists():
...
if summary_rows:
    ...
    df_summary.to_csv(OUT_ROOT / "test_summary_all_folds.csv", index=False)
